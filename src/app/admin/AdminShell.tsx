    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="flex flex-col items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-burgundy-500 to-burgundy-400 flex items-center justify-center">
            <Sparkles className="w-5 h-5 text-white animate-pulse" />
          </div>
          <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-burgundy-500" />
        </div>
      </div>
    )
  }
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-5 border-b border-slate-800/80">
          <Link href="/admin" className="flex items-center gap-2.5 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-burgundy-500 to-burgundy-400 flex items-center justify-center shadow-lg shadow-burgundy-500/20 group-hover:shadow-burgundy-500/40 transition-shadow">
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-burgundy-500/15 text-burgundy-400 shadow-sm shadow-burgundy-500/5"
              ...
              >
                ...
                {item.href === "/admin/chat" && chatUnread > 0 && !isActive && (
                  <span className="ml-auto inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-burgundy-500 text-white text-[10px] font-bold">
                    {chatUnread > 9 ? "9+" : chatUnread}
                  </span>
                )}
                {isActive && <ChevronRight className="w-3.5 h-3.5 ml-auto shrink-0 text-burgundy-400" />}
              </Link>
            )
          })}
        </nav>
        ...
        <header className="h-16 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md flex items-center justify-between px-4 lg:px-6 shrink-0 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 h-9 px-3 rounded-lg bg-slate-100 dark:bg-slate-800/80 text-xs text-slate-600 dark:text-slate-400">
              <div className="w-5 h-5 rounded-full bg-gradient-to-br from-burgundy-400 to-burgundy-400 flex items-center justify-center">
