  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">New Deal</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Add a new special offer</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-6 space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label>Title *</Label>
              <Input value={form.title} onChange={(e) => handleTitleChange(e.target.value)} placeholder="e.g. Early Bird Safari" required />
            </div>
            <div className="space-y-2">
              <Label>Slug *</Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.duration} onChange={(e) => setForm((p) => ({ ...p, duration: e.target.value }))} placeholder="e.g. 5 Days / 4 Nights" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input type="number" value={form.original_price} onChange={(e) => setForm((p) => ({ ...p, original_price: e.target.value }))} placeholder="0" />
            </div>
            <div className="space-y-2">
              <Label>Deal Price (USD)</Label>
              <Input type="number" value={form.deal_price} onChange={(e) => setForm((p) => ({ ...p, deal_price: e.target.value }))} placeholder="0" />
            </div>
            <div className="space-y-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={form.discount} onChange={(e) => setForm((p) => ({ ...p, discount: e.target.value }))} placeholder="e.g. 20% Off" />
            </div>
            <div className="space-y-2">
              <Label>Promo Code</Label>
              <Input value={form.code} onChange={(e) => setForm((p) => ({ ...p, code: e.target.value }))} placeholder="e.g. EARLY20" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={form.valid_until} onChange={(e) => setForm((p) => ({ ...p, valid_until: e.target.value }))} placeholder="e.g. Dec 31, 2026" />
            </div>
            <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="featured" className="text-sm font-normal">Featured deal (shown prominently)</Label>
          </div>
