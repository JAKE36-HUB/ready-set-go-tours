  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          { label: "Impressions", value: totals.impressions.toLocaleString(), icon: Eye, color: "text-burgundy-500" },
          { label: "Clicks", value: totals.clicks.toLocaleString(), icon: MousePointerClick, color: "text-burgundy-500" },
          { label: "WhatsApp Clicks", value: totals.whatsappClicks.toLocaleString(), icon: TrendingUp, color: "text-burgundy-500" },
          { label: "Conversions", value: totals.conversions.toLocaleString(), icon: Trophy, color: "text-burgundy-500" },
          { label: "Est. Revenue", value: `$${totals.revenue.toLocaleString()}`, icon: Wallet, color: "text-burgundy-500" },
        ].map((s) => (
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {best && (
          <div className="rounded-xl border border-burgundy-200 dark:border-burgundy-900 bg-burgundy-50/50 dark:bg-burgundy-950/30 p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-burgundy-500 flex items-center justify-center shrink-0">
              <TrendingUp className="w-5 h-5 text-white" />
            </div>
            <div className="min-w-0">
              <p className="text-xs text-burgundy-600 dark:text-burgundy-400 font-medium">Best Performing</p>
                ...
                {popups.map((p) => (
                  <tr key={p.id + p.variant} className="border-b border-slate-50 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    <td className="px-4 py-2.5">
                      ...
                      {p.variantOf ? (
                        <span className={cn("ml-2 text-[10px] font-bold px-1.5 py-0.5 rounded", p.variant === "B" ? "bg-burgundy-100 text-burgundy-700 dark:bg-burgundy-950 dark:text-burgundy-400" : "bg-burgundy-100 text-burgundy-700 dark:bg-burgundy-950 dark:text-burgundy-400")}>
                          {p.variant}
                        </span>
                      ) : null}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-right text-burgundy-500 tabular-nums">{p.whatsappClicks}</td>
                    <td className={cn("px-3 py-2.5 text-right font-semibold tabular-nums", p.ctr >= 5 ? "text-burgundy-600 dark:text-burgundy-400" : "text-slate-600 dark:text-slate-300")}>
                      {p.ctr.toFixed(1)}%
                    </td>
                    <td className="px-3 py-2.5 text-right text-burgundy-600 dark:text-burgundy-400 tabular-nums">{p.conversions}</td>
                    ...
                    <td className="px-3 py-2.5 text-right text-burgundy-600 dark:text-burgundy-400 font-semibold tabular-nums">${p.revenue.toLocaleString()}</td>
                  </tr>
                ))}
                {popups.length === 0 && (
            ...
            return (
              <div key={g.id} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-burgundy-600 dark:text-burgundy-400 font-semibold">
                    {winner.variant} winning by {lift.toFixed(1)}%
                  </span>
                </div>
                <div className="grid md:grid-cols-2 gap-4">
                  {[a, b].map((v) => (
                    <div key={v.variant} className={cn("rounded-lg border p-4", winner.variant === v.variant ? "border-burgundy-300 dark:border-burgundy-800 bg-burgundy-50/40 dark:bg-burgundy-950/20" : "border-slate-200 dark:border-slate-700")}>
                      <div className="flex items-center justify-between mb-2">
                        <span className={cn("text-xs font-bold px-2 py-0.5 rounded", v.variant === "B" ? "bg-burgundy-100 text-burgundy-700 dark:bg-burgundy-950 dark:text-burgundy-400" : "bg-burgundy-100 text-burgundy-700 dark:bg-burgundy-950 dark:text-burgundy-400")}>
                          Variant {v.variant}
                        </span>
                        {winner.variant === v.variant && <span className="text-[10px] font-bold text-burgundy-600 dark:text-burgundy-400">🏆 WINNER</span>}
                      </div>
