...
import { ArrowLeft, Plus, X, Save, MapPin, ImageIcon, Info, Star, Sparkles } from "lucide-react"
...
export default function EditDestinationPage() {
      ...
      try {
        ...
        if (json.data) {
          ...
          setForm({
            ...
            rating: String(d.rating ?? ""), best_time: d.best_time || "", duration: d.duration || "",
            starting_price: String(d.starting_price ?? ""), region: d.region || "kenya",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      else { const err = await res.json(); alert(err.error || "Failed to update destination") }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-600 via-emerald-500 to-teal-400 p-6">
        ...
        <div className="relative z-10 flex items-center gap-4">
          <button onClick={() => router.push("/admin/destinations")} className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-bold text-white truncate">Edit Destination</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-2"><Label>Rating (1-5)</Label><Input type="number" min="0" max="5" step="0.5" value={form.rating} onChange={(e) => setForm((p) => ({ ...p, rating: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Starting Price ($)</Label><Input type="number" value={form.starting_price} onChange={(e) => setForm((p) => ({ ...p, starting_price: e.target.value }))} /></div>
            </div>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden">
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2"><Label>Best Time to Visit</Label><Input value={form.best_time} onChange={(e) => setForm((p) => ({ ...p, best_time: e.target.value }))} /></div>
              <div className="space-y-2"><Label>Typical Duration</Label><Input value={form.duration} onChange={(e) => setForm((p) => ({ ...p, duration: e.target.value }))} /></div>
            </div>
        ...
        <div className="flex justify-end gap-3 pb-8">
          <Button variant="outline" onClick={() => router.push("/admin/destinations")} className="border-slate-200 dark:border-slate-700">Cancel</Button>
          <Button type="submit" disabled={saving}
            className="bg-gradient-to-r from-emerald-500 to-teal-400 text-white border-0 hover:shadow-lg hover:shadow-emerald-500/20">
