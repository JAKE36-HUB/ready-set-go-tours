  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Tour Packages</h2>
                ...
                <tr key={pkg.id} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {pkg.image && (
                        <img src={pkg.image} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0" />
                      )}
                      <span className="text-sm font-medium text-slate-900 dark:text-white">{pkg.name}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3.5 hidden sm:table-cell">
                    <Badge className={cn("text-xs font-medium", TYPE_COLORS[pkg.type] || "bg-slate-100 text-slate-700")}>
                      {pkg.type}
                    </Badge>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="icon" onClick={() => window.open(`/holiday-packages/${pkg.slug}`, "_blank")} aria-label="View on site">
                        <ExternalLink className="w-3.5 h-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => router.push(`/admin/packages/${pkg.id}/edit`)} aria-label="Edit package">
