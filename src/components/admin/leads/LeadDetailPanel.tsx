      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          <div className="grid grid-cols-3 gap-2">
            {lead.phone && (
              <a href={`tel:${lead.phone}`} className="flex flex-col items-center gap-1 rounded-xl bg-burgundy-50 dark:bg-burgundy-950/50 text-burgundy-700 dark:text-burgundy-400 py-2.5 hover:bg-burgundy-100 dark:hover:bg-burgundy-950 transition-colors">
                <Phone className="w-4 h-4" /><span className="text-[10px] font-semibold">Call</span>
              </a>
            )}
            {phoneDigits && (
              <a href={whatsappNumber} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center gap-1 rounded-xl bg-burgundy-50 dark:bg-burgundy-950/50 text-burgundy-700 dark:text-burgundy-400 py-2.5 hover:bg-burgundy-100 dark:hover:bg-burgundy-950 transition-colors">
                <MessageCircle className="w-4 h-4" /><span className="text-[10px] font-semibold">WhatsApp</span>
              </a>
            )}
            {lead.email && (
              <a href={`mailto:${lead.email}`} className="flex flex-col items-center gap-1 rounded-xl bg-burgundy-50 dark:bg-burgundy-950/50 text-burgundy-700 dark:text-burgundy-400 py-2.5 hover:bg-burgundy-100 dark:hover:bg-burgundy-950 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button disabled={!!busy} onClick={() => patch({ status: "quote_sent" }, "quote_sent")} className="flex items-center justify-center gap-1.5 rounded-xl bg-burgundy-100 dark:bg-burgundy-950/60 text-burgundy-700 dark:text-burgundy-400 py-2.5 text-xs font-semibold hover:bg-burgundy-200 dark:hover:bg-burgundy-900 transition-colors disabled:opacity-50">
              <Send className="w-3.5 h-3.5" /> Qualified
            </button>
            <button disabled={!!busy} onClick={() => patch({ status: "booked" }, "booked")} className="flex items-center justify-center gap-1.5 rounded-xl bg-burgundy-100 dark:bg-burgundy-950/60 text-burgundy-700 dark:text-burgundy-400 py-2.5 text-xs font-semibold hover:bg-burgundy-200 dark:hover:bg-burgundy-900 transition-colors disabled:opacity-50">
              <Plane className="w-3.5 h-3.5" /> Booked
            </button>
            <button disabled={!!busy} onClick={() => patch({ archived: !lead.archived }, "archive")} className={cn("flex items-center justify-center gap-1.5 rounded-xl py-2.5 text-xs font-semibold transition-colors disabled:opacity-50", lead.archived ? "bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700" : "bg-burgundy-100 dark:bg-burgundy-950/60 text-burgundy-700 dark:text-burgundy-400 hover:bg-burgundy-200 dark:hover:bg-burgundy-900")}>
            ...
            <div className="space-y-1.5">
              {(lead.reminders || []).map((r) => (
                <div key={r.id} className="flex items-center gap-2 rounded-lg bg-slate-50 dark:bg-slate-800/60 px-2.5 py-2">
                  <button
                    ...
                    className={cn("shrink-0 w-4 h-4 rounded border flex items-center justify-center transition-colors", r.done ? "bg-burgundy-500 border-burgundy-500" : "border-slate-300 dark:border-slate-600 hover:border-burgundy-400")}
                  >
              ...
              <div className="pt-1 space-y-1.5">
                <div className="flex gap-1.5">
                  {["Call tomorrow", "Email later", "Follow up in 3 days"].map((preset) => (
                    <button
                      ...
                      className="flex-1 text-[10px] rounded-lg border border-slate-200 dark:border-slate-700 px-2 py-1.5 text-slate-500 hover:border-burgundy-400 hover:text-burgundy-600 transition-colors"
                    >
                      {preset}
                    </button>
                  ))}
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={addReminder} disabled={reminderSaving || !reminderTitle || !reminderDate} className="rounded-lg bg-burgundy-500 text-white px-2.5 py-1.5 hover:bg-burgundy-600 transition-colors disabled:opacity-40">
            ...
            <div className="space-y-3">
              {(lead.events || []).map((ev) => (
                <div key={ev.id} className="relative pl-4">
                  <span className="absolute left-0 top-1 w-2 h-2 rounded-full bg-burgundy-400" />
