        ...
        <>
              ...
              >
                ...
                <div className="relative px-5 pt-5 pb-5">
                  ...
                  <button
                    ...
                    className="mt-4 w-full h-10 rounded-xl gradient-primary text-white text-xs font-bold tracking-wide shadow-lg shadow-burgundy-900/25 hover:shadow-burgundy-900/40 hover:scale-[1.02] transition-all"
                  >
                    Open Live Chat
                  </button>
                </div>
          ...
          >
            ...
            <span className="relative inline-flex w-14 h-14 shrink-0">
              ...
              <span className="relative flex w-14 h-14 items-center justify-center rounded-full gradient-primary text-white shadow-lg shadow-burgundy-900/30 group-hover:scale-110 group-hover:shadow-burgundy-900/50 transition-all duration-300">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 w-[380px] max-w-[calc(100vw-2rem)] h-[560px] max-h-[calc(100vh-170px)] bg-[#FAF7F2] dark:bg-stone-950 rounded-3xl shadow-2xl shadow-stone-900/25 ring-1 ring-stone-900/10 dark:ring-stone-600/40 flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="relative shrink-0 overflow-hidden px-4 pt-4 pb-3 gradient-primary">
              ...
              <div className="relative flex items-center gap-3">
                <div className="relative shrink-0">
                  ...
                  <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-300 ring-2 ring-white/40 animate-pulse" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-display text-[15px] font-bold text-white leading-tight tracking-tight">Ready Set Go Safaris</p>
            ...
            {/* Messages */}
            <div ref={chatScrollRef} onScroll={handleChatScroll} className="flex-1 overflow-y-auto px-4 py-4 bg-gradient-to-b from-burgundy-50/60 via-[#FAF7F2] to-[#FAF7F2] dark:from-stone-900 dark:via-stone-950 dark:to-stone-950 [scrollbar-width:thin] overscroll-contain">
              <div className="space-y-3">
                  ...
                  {messages.map((msg) => (
                    ...
                    >
                      {msg.role !== "user" && (
                        <span
                          className={`shrink-0 mt-1 w-7 h-7 rounded-full flex items-center justify-center shadow-md ${
                            msg.role === "owner"
                              ? "bg-gradient-to-br from-burgundy-600 to-burgundy-400 text-white shadow-burgundy-500/25"
                              : "bg-gradient-to-br from-burgundy-600 to-burgundy-400 text-white shadow-burgundy-500/25"
                          }`}
                        >
                      ...
                      <div className={`flex flex-col max-w-[78%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
                        {msg.role === "owner" && (
                          <span className="inline-flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wider text-burgundy-600 dark:text-burgundy-400 mb-1">
                            Ready Set Go Team
                          </span>
                        )}
                        <div
                          className={`whitespace-pre-wrap break-words text-sm leading-relaxed px-4 py-2.5 rounded-2xl ${
                            msg.role === "user"
                              ? "bg-gradient-to-br from-burgundy-600 to-burgundy-500 text-white shadow-lg shadow-burgundy-900/25 rounded-br-sm"
                              : msg.role === "owner"
                                ...
                                : "bg-white dark:bg-stone-800 ring-1 ring-burgundy-900/10 dark:ring-burgundy-400/15 text-stone-800 dark:text-stone-200 shadow-sm rounded-bl-sm"
                          }`}
                        >
                          {msg.content}
                        </div>
                ...
                {loading && (
                  ...
                  >
                    <span className="shrink-0 mt-1 w-7 h-7 rounded-full bg-gradient-to-br from-burgundy-600 to-burgundy-400 flex items-center justify-center text-white shadow-md shadow-burgundy-500/25">
                      <HeadphonesIcon className="w-3.5 h-3.5" />
                    </span>
                    <div className="rounded-2xl rounded-bl-sm px-4 py-3 bg-white dark:bg-stone-800 ring-1 ring-burgundy-900/10 dark:ring-burgundy-400/15 shadow-sm">
                      <span className="flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-burgundy-500 animate-bounce" />
                        <span className="w-1.5 h-1.5 rounded-full bg-burgundy-500 animate-bounce [animation-delay:0.15s]" />
                        <span className="w-1.5 h-1.5 rounded-full bg-burgundy-500 animate-bounce [animation-delay:0.3s]" />
                      </span>
                    </div>
                  </motion.div>
                )}
                ...
                {awaitingTeam && (
                  <div className="flex items-end gap-2">
                    ...
                    <div className="rounded-2xl rounded-bl-sm px-4 py-3 bg-burgundy-50 dark:bg-burgundy-950/70 ring-1 ring-burgundy-200 dark:ring-burgundy-500/30">
                      <div className="flex items-center gap-2 text-sm text-burgundy-800 dark:text-burgundy-200">
            ...
            {/* Input */}
            <div className="shrink-0 px-3 pt-2 pb-2.5 bg-[#FAF7F2] dark:bg-stone-950 border-t border-stone-900/10 dark:border-stone-600/30">
              <div className="flex items-center gap-2 pl-4 pr-1.5 py-1.5 rounded-2xl bg-[#F1EAE0] dark:bg-stone-800 ring-1 ring-stone-900/10 dark:ring-stone-600/50 focus-within:ring-2 focus-within:ring-burgundy-500/40 transition-all">
                <input
                  ref={inputRef}
                  type="text"
                  value={input}
                  ...
                  className="flex-1 h-9 bg-transparent text-sm text-stone-900 dark:text-white placeholder:text-stone-400 focus:outline-none border-0"
                  aria-label="Type your travel question"
                />
                <button
                  onClick={() => sendMessage()}
                  disabled={loading || !input.trim()}
                  className="w-9 h-9 rounded-xl bg-gradient-to-br from-burgundy-600 to-burgundy-500 text-white flex items-center justify-center shrink-0 shadow-md shadow-burgundy-900/25 hover:scale-105 transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
                  aria-label="Send message"
                >
                  <Send className="w-4 h-4" />
                </button>
              </div>
              <p className="mt-2 text-[10px] text-stone-500 flex items-center justify-center gap-1">
